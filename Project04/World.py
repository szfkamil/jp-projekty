...
class World(object):
	...
	def __init__(self, worldX, worldY):
		...
		self.__newOrganisms = []
		self.__separator = '.'
		self.__plagueTurns = 0 # plague turn counter
	...
	def triggerPlague(self): # new function
		self.__plagueTurns = 2
		for o in self.organisms:
			o.liveLength = int(o.liveLength / 2) # force integer division
	...
	def makeTurn(self):
		...
		# new condition, so that the plague only lasts two turns
	...
	def addOrganism(self, newOrganism):
		# receieve newly spawned object with its coordinates
		newOrgPosition = Position(xPosition=newOrganism.position.x, yPosition=newOrganism.position.y)

		# ensures that the coordinates are on the board
		if self.positionOnBoard(newOrgPosition):
			self.organisms.append(newOrganism) # adds organism to the world
			self.organisms.sort(key=lambda org: org.initiative, reverse=True) # sorts  the list of animals by initiative (descending)
			return True
		return False
