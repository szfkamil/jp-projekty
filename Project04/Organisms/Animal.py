...
class Animal(Organism):
	...
	def move(self):
		...
		if pomPositions:
			...
			metOrganism = self.world.getOrganismFromPosition(newPosition) # pyright: ignore[reportOptionalMemberAccess]
	...
	def action(self):
		...
		if self.ifReproduce() and birthPositions:
			...
			newAnimal.initParams() # type: ignore
			newAnimal.position = newAnimalPosition # type: ignore
			self.power = self.power / 2 # type: ignore
			result.append(Action(ActionEnum.A_ADD, newAnimalPosition, 0, newAnimal))
		return result

	def getNeighboringPosition(self):
		return self.world.getNeighboringPositions(self.position) # type: ignore

	def getNeighboringBirthPosition(self):
		return self.world.filterFreePositions(self.world.getNeighboringPositions(self.position)) # type: ignore
